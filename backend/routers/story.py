...
def generate_story_task(job_id: str, theme: str, session_id: str):
    ...
    try:
        ...
        try:
            ...
            story= StoryGenerator.generate_story(db=db,theme=theme, session_id=session_id)

            job.story_id =story.id  # type: ignore # TODO: Update story id
...
def build_complete_story_tree(db: Session, story: Story) -> CompleteStoryResponse: # type: ignore
    ...
    root_node = next((node for node in nodes if node.is_root), None)    # type: ignore
    if not root_node:
        raise HTTPException(status_code=500, detail="Story root node is not found")
